...
const INIT_STATE = {
  ...
  productsCountInCart: JSON.parse(localStorage.getItem("cart"))
    ...
    : 0,
  productsCountInFavorites: JSON.parse(localStorage.getItem("favorites"))
  ...
  //product list
    ...
    const getProducts = async () => {
      const { data } = await axios(
        `${API_PRODUCTS}/products${window.location.search.replace(
          /%3D/,
          ""
        )}`
      );
...
//   const URI = "https://kitep-1dd76-default-rtdb.firebaseio.com/products.json";
//   AXIOS.getProduct(URI).then((data) => {
//     dispatch({
//       type: "GET_PRODUCTS_DATA",
//       payload: data,
//     });
//   });
  ...
  async function getProductsBusiness() {
    const { data } = await axios(
      `${API_PRODUCTS}/products?genre=business&limit=3`
    );
    dispatch({
  ...
  async function getProductsClassicLiterature() {
    const { data } = await axios(
      `${API_PRODUCTS}/products?genre=classicLiterature`
    );
    console.log(data);
    dispatch({
  ...
  async function getProductsChildren() {
    const { data } = await axios(
      `${API_PRODUCTS}/products?genre=childrensBook`
    );
    dispatch({
  ...
  async function sendComment(product, value) {
    ...
    await axios.patch(`${API_PRODUCTS}/products/${product.id}.json`, product);
    getProducts();
  }
